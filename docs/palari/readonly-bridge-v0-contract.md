...
- [ ] T3 is confirmed at base `c1ec1915fc16f3dc1ec5d47d9a97f6210a574526`,
...
- [ ] The sibling Company OS checkout is confirmed at
...
- [ ] Repository instructions, package roles, package manager, required Node
...
- [ ] Exact Company OS CLI inputs and observed JSON schemas are documented
      without invented fields or commands.
      Evidence: `docs/palari/company-os-orientation.md`.

## Pinned compatibility

- [ ] The adapter hard-pins Company OS revision
...
- [ ] A wrong Git revision, wrong workspace schema, wrong packet schema, and
...
- [ ] Queue and both ready and human-blocked packet reads leave every fixture
      file unchanged.
      Evidence: `docs/palari/evidence/fixture-fingerprint.json`.
- [ ] The expected normalized overview contains none of the prohibited raw
      CLI fields.
...
- [ ] Contract tests cover capability version skew, unknown variants, response
      bounds, and truncation indicators.
      Evidence: `docs/palari/evidence/checks.md`.
- [ ] Query and UI tests cover caching, refresh, persistence migration,
      attention ordering, all display states, and absence of mutation controls.
      Evidence: `docs/palari/evidence/checks.md`.

## Security and redaction

- [ ] Browser payloads contain no local checkout/workspace path, source URI,
...
- [ ] Sensitive-looking errors and environment values are redacted under every
      process failure mode.
      Evidence: redaction tests.
- [ ] No `.env`, secret, live system, provider, customer content, or real
      Company OS workspace was accessed.
      Evidence: independent review and founder packet.

## Accessibility and responsive behavior

- [ ] The panel is semantically labelled, exposes loading/status/alert states,
      and is usable from the keyboard.
      Evidence: Palari-only Chromium tests.
- [ ] Refresh works by Tab/Enter, and the mobile sheet closes with Escape and
      restores focus.
      Evidence: Palari-only Chromium tests.
- [ ] Desktop 1440×900 and mobile 390×844 renders have no overlap, clipping, or
...
- [ ] No runtime dependency is added and the panel remains lazy-loaded.
      Evidence: lockfile/package diff and `bundle-impact.json`.
- [ ] Relative to the clean pinned base, initial JS grows by at most 5 KiB
...
- [ ] Existing T3 behavior and persisted right-panel data migrate safely when
      Palari is disabled or an older server is used.
      Evidence: regression and version-skew tests.

## Required verification

- [ ] Pinned Company OS `./scripts/verify.sh` and offline demo pass.
      Evidence: `docs/palari/evidence/company-os-verification.md`.
- [ ] `vp check` passes.
      Evidence: `docs/palari/evidence/checks.md`.
- [ ] `vp run typecheck` passes.
      Evidence: `docs/palari/evidence/checks.md`.
- [ ] `vp run test` and targeted Palari tests pass.
      Evidence: `docs/palari/evidence/checks.md`.
- [ ] Relevant production builds and the Palari-only Chromium project pass.
      Evidence: checks, screenshots, and bundle evidence.

## Review and founder decision packet

- [ ] A fresh independent reviewer returns `ACCEPT` after all substantive
      findings are repaired.
      Evidence: `docs/palari/evidence/independent-review.md`.
- [ ] Residual risks and intentionally deferred capabilities are explicit.
      Evidence: `docs/palari/founder-packet-v0.md`.
- [ ] The founder packet contains exact pins, architecture, verification,
      screenshots, bundle impact, risks, deferred APP/GitHub mapping, and a
      recommended next phase.
      Evidence: `docs/palari/founder-packet-v0.md`.
- [ ] Every checkbox above is supported by committed evidence; the branch has
